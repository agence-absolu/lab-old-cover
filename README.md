...
Une démo de plus dans l’univers du Lab Absolu.

Démo du lab Absolu, publiée sur **https://lab.agence-absolu.com/old-cover/**.

Titre en lettres rouges sur fond noir, une vidéo en `mix-blend-mode: darken`
n'apparaît que dans les lettres ; elles grossissent jusqu'à ce que leur rouge
recouvre toute l'image. Portage du CodePen
[PoxgENR](https://codepen.io/deodat/pen/PoxgENR) (GSAP + SplitText, grain
animé sur canvas). Un clic rejoue l'animation.

L'extrait vidéo (`src/assets/detour.mp4`, 12 s, muet) vient de *Detour*
(Edgar G. Ulmer, 1945), film noir du domaine public, copie
[archive.org/details/Detour_66](https://archive.org/details/Detour_66).

## Développement

```bash
npm install
npm run dev       # http://localhost:5173/old-cover/
...
base des chemins du nom npm (`old-cover`). `BASE_PATH=/ npm run build` pour une
racine de domaine.

## Publication

`.github/workflows/deploy.yml` compile et envoie `dist/` par rsync sur SSH dans
`lab.agence-absolu.com/lab-projects/old-cover/` à chaque push sur `main` — ou à
la demande, onglet Actions. Le hub sert ce dossier sur `/old-cover/` sans
aucune configuration de son côté.

### 1. Le dépôt

```bash
gh repo create agence-absolu/lab-old-cover --public --source=. --push
```
